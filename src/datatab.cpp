...
void DataTab::appendSendedData(const QByteArray& data)
{
    ...
    if(isSendedDataHex)
    {
        ui->sendedEdit->insertPlainText(curTime+data.toHex(' ') + '\n');
...
//        }
    ...
    {
        ui->sendedEdit->insertPlainText(curTime+ dataCodec->toUnicode(data)+ '\n');
    }
}
...
void DataTab::appendReceivedData(const QByteArray& data)
{
    ...
    if(isReceivedDataHex)
    {
        ui->receivedEdit->insertPlainText(curTime+data.toHex(' ') + '\n');
...
//        }
    ...
    else
    {
        ...
        if(lastReceivedByte == '\r' && !data.isEmpty() && *data.cbegin() == '\n')
            ui->receivedEdit->insertPlainText(curTime+RxDecoder->toUnicode(data.right(data.size() - 1))+ '\n');
        else
            ui->receivedEdit->insertPlainText(curTime+RxDecoder->toUnicode(data)+ '\n');
